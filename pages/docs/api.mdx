...
The `shadow` utility includes style props for `textShadow` and `boxShadow`. It will return a value in the theme's `shadows` property, else it will attempt to return a value in the `colors` property if the color is specified at the end of the value.
